...
export const humanDesignEventDiaryCorrelations = (chart, events = []) => {
  const analysis = humanDesignEventCorrelations(chart, events)
  const eventIdsForGate = gate => analysis.events
    .filter(event => event.gates.includes(gate))
    .map(event => event.id || event.dateMs || `event-${analysis.events.indexOf(event) + 1}`)
  const eventGatesForTag = tag => sortedUnique(analysis.events
    .filter(event => event.tags.includes(tag))
    .flatMap(event => event.gates))

  return {
    ...analysis,
    gateCorrelations: analysis.repeatedGates.map(row => ({
      ...row,
      eventIds: eventIdsForGate(row.gate),
      summary: `Gate ${row.gate} appears in ${row.count} event record(s), making it useful for diary pattern review.`,
    })),
    tagCorrelations: analysis.repeatedTags.map(row => ({
      ...row,
      gates: eventGatesForTag(row.tag),
      summary: `${row.tag} events repeat gates ${eventGatesForTag(row.tag).join(', ') || 'none yet'}.`,
    })),
    lineCorrelations: analysis.repeatedLines,
    circuitCorrelations: sortedUnique(analysis.events.flatMap(event =>
      event.completedNatalChannels.flatMap(channel => channel.circuit || [])
    )).map(circuit => ({
      circuit,
      count: analysis.events.filter(event => event.completedNatalChannels.some(channel => channel.circuit === circuit)).length,
      summary: `${circuit} circuitry appears through event channel completions.`,
    })),
  }
}

export const humanDesignLinePatternAnalysis = chart => {
  const analysis = linePatternAnalysis(chart)
  const lineCounts = Object.fromEntries(analysis.counts.map(row => [row.line, row.count]))
  return {
    ...analysis,
    lineCounts,
    dominantLines: analysis.dominantLines.map(row => ({
      ...row,
      role: lineLibraryEntry(row.line, row.line).role,
      summary: row.note,
    })),
    profileResonance: {
      profile: chart?.profile || '',
      profileLines: analysis.profileLines,
      lines: analysis.profileResonance,
    },
  }
}

export const humanDesignPlanetGateWeighting = chart => {
  const analysis = planetGateWeighting(chart)
  const layerTotals = analysis.byPlanet.reduce((totals, row) => ({
    ...totals,
    [row.layer]: Number(((totals[row.layer] || 0) + row.weight).toFixed(2)),
  }), {})
  return {
    ...analysis,
    planetWeights: analysis.weights,
    weightedGates: analysis.rankedGates.map(row => ({
      ...row,
      sources: row.activations,
    })),
    layerTotals,
  }
}
...
export const humanDesignHarmonicCompletionTiming = (chart, opts = {}) => {
  const base = harmonicCompletionTiming(chart, opts.transitChart || null)
  const start = opts.dateMs || Date.now()
  const completions = base.hangingGates.slice(0, Math.max(1, opts.limit || 12)).map((row, index) => ({
    natalGate: row.gate,
    transitGate: row.harmonicGate,
    channel: row.channel,
    dateMs: start + (index * 24 * 60 * 60 * 1000),
    summary: `Watch transit gate ${row.harmonicGate} for completion of channel ${row.channel} from natal gate ${row.gate}.`,
  }))
  return {
    ...base,
    completions,
...
export const humanDesignCircuitStreamBalance = chart => {
  const analysis = circuitStreamBalance(chart)
  const summarize = row => ({
    ...row,
    summary: `${row.key} carries ${row.count} channel(s), ${row.percent}% of defined channel emphasis.`,
  })
  return {
    ...analysis,
    circuitGroups: analysis.groups.map(summarize),
    circuits: analysis.circuits.map(summarize),
    streams: analysis.streams.map(summarize),
    dominantCircuit: analysis.dominantCircuit ? summarize(analysis.dominantCircuit) : null,
  }
}

export const humanDesignIncarnationCrossResonance = (chart, transitChart = null) => {
  const analysis = incarnationCrossResonance(chart, transitChart)
  const transitGates = chartGateSet(transitChart)
  const crossGates = analysis.gates.map(row => row.gate)
  const gateResonance = analysis.gates.map(row => ({
    gate: row.gate,
    role: chart?.incarnationCross?.roles?.find(role => role.gate === row.gate)?.key || 'cross gate',
    natalDefined: row.definedInNatal,
    transitActive: transitGates.has(row.gate),
    summary: `Gate ${row.gate} is part of the cross frame and is ${transitGates.has(row.gate) ? 'active' : 'not active'} by current transit.`,
  }))
  const resonanceScore = crossGates.length
    ? Number((gateResonance.filter(row => row.natalDefined || row.transitActive).length / crossGates.length).toFixed(2))
    : 0
  return {
    ...analysis,
    crossGates,
...
export const humanDesignVariableConsistencyChecks = (chart, opts = {}) => {
  const precision = opts.birthTimePrecisionMinutes !== undefined
    ? `${opts.birthTimePrecisionMinutes} minutes`
    : opts.birthTimePrecision
  const analysis = variableConsistencyChecks(chart, { ...opts, birthTimePrecision: precision })
  const fragile = opts.birthTimePrecisionMinutes === undefined ? false : opts.birthTimePrecisionMinutes > 5
  return {
    ...analysis,
    reliable: analysis.reliable && !fragile,
    orientationPattern: (chart?.variables || []).map(variable =>
      variable.orientation === 'left' ? 'L' : variable.orientation === 'right' ? 'R' : '?'
    ).join(''),
    ...
    warnings: [
      ...analysis.warnings,
      fragile ? 'Variable analysis is sensitive to birth time precision; use exact birth time before treating it as stable.' : '',
    ].filter(Boolean),
  }
}

export const humanDesignRelationshipActivationOverlay = (chartA, chartB) => {
  const analysis = humanDesignConnectionCorrelations([chartA, chartB])
  const toCompletion = (owner, activator, row) => ({
    ownerId: owner?.personId || '',
    activatorId: activator?.personId || '',
    natalGate: row.sourceGate,
    activatorGate: row.targetGate,
    channel: row.channel,
    summary: `${activator?.personName || 'The other chart'} completes ${row.channel} for ${owner?.personName || 'this chart'}.`,
  })
  return {
    ...analysis,
    completedHangingGates: [
      ...(analysis.leftCompletesRight || []).map(row => toCompletion(chartA, chartB, row)),
      ...(analysis.rightCompletesLeft || []).map(row => toCompletion(chartB, chartA, row)),
    ],
    openCenterTouches: analysis.openCenterSensitivity || [],
    crossGateContacts: [
      ...(analysis.leftActivatesRightCross || []).map(row => ({ ...row, activatorId: chartA?.personId, ownerId: chartB?.personId })),
      ...(analysis.rightActivatesLeftCross || []).map(row => ({ ...row, activatorId: chartB?.personId, ownerId: chartA?.personId })),
    ],
  }
}

export const humanDesignTransitThemeClusters = (natalChart, transitChart) => {
  const analysis = humanDesignTransitCorrelations(natalChart, transitChart)
  const byLine = (analysis.todayThemes.lines || []).map(line => ({
    key: `line-${line.line}`,
    line: line.line,
    count: line.count,
    gates: line.gates,
    summary: `Line ${line.line} has ${line.count} current transit activation(s).`,
  }))
  const byCenter = (analysis.todayThemes.centers || []).map(center => ({
    key: center.center,
    center: center.center,
    count: center.count,
    gates: center.gates,
    summary: `${center.center} carries ${center.count} current transit gate(s).`,
  }))
  const clusters = [...byCenter, ...byLine]
  return {
    ...analysis,
    todayThemes: analysis.todayThemes.gates,
    byCenter,
    byLine,
    completedChannels: analysis.completedChannels,
    clusters,
  }
}

export const humanDesignAstrologyBridge = (chart, astrologyAspects = []) => {
  const bridge = bridgeAstrologyHumanDesign(chart, astrologyAspects)
  const entries = bridge.rows.map(row => {
    const first = row.activations[0]
    return {
      ...row.aspect,
      planet: row.planets[0] || row.aspect?.planet || '',
      gate: first?.activation?.gate || 0,
      line: first?.activation?.line || 0,
      code: first?.code || '',
      gateSummary: first?.gate?.summary || '',
      astrologySummary: `${row.planets.join(' / ') || 'This aspect'} is read through the supplied zodiac aspect context.`,
      combinedSummary: row.summary,
    }
  })
  return {
    ...bridge,
